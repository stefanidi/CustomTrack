...
//			_beaconManager.SetBackgroundMode(false);
		...
		private async Task UpdateData(List<Beacon> beacons)
		{
			...
			{	
                ...
                //Here we send Beacon Data To Server
                var allBeacons = _data.Concat(newBeacons).ToList();
                var val = allBeacons.Select(a => a.Distance.ToString()).FirstOrDefault();
                //(new WebClient()).UploadStringAsync(new Uri("http://customtrack.azurewebsites.net/api/Reporting/Boom/?data=" + val), "GET", "");
                //(new WebClient()).UploadStringAsync(new Uri("http://customtrack.azurewebsites.net/api/Reporting/Record/"), "POST", val);
                (new WebClient()).DownloadDataAsync(new Uri("http://customtrack.azurewebsites.net/api/Reporting/Boom/?data=" + val));
		...
		{
			_beaconManager.SetForegroundBetweenScanPeriod(500); // 5000 milliseconds
